...
//					  divJq.attr("xpl-dc-val",conditionObj.conditionValueVal);
    	...
    	instance.buildCacheCondition = function(){
            ...
            // layer.msg(typeof(conditionRowJqs))
    		...
    		for(var i=0;i<rowLength;i++){
    			...
    			var curEditor = dynamicCondition.editor[item.edit];
	  			if(curEditor){
	  				...
	  				conditionObj.conditionValueVal = tempjq ? curEditor.getRequestValue(tempjq) : null;
	  				tempjq = conditionRowDOM.eleLeftJq;
	    			conditionObj.conditionValueLeftVal = tempjq ?curEditor.getRequestValue(tempjq) : null;
	    			tempjq = conditionRowDOM.eleRightJq;
	    			conditionObj.conditionValueRightVal = tempjq ? curEditor.getRequestValue(tempjq) : null;
                  }
                  conditionObj.logic = $("#logic[name='logic']").val();
                  //获取查询逻辑
                  conditionObj.gaoJi = 'true';
    	...
    	instance.buildConditionHtml = function(){
    		...
    		for(var i=0;i<cacheCondition.length;i++){
    			...
    			// alert(fieldText+OptionText+ValueText+ValueLeftText+ValueRightText)
    			var rsValueText="";
...
//    		  content: conditionContainerHtml
    	...
    	instance.render = function(divJq){
    		...
    		var conditionContainerHtml = '<div id="'+instance.conditionContainerId+'" class="conditionContainer" lay-filter="conditionContainer"><div class="layui-xpl-dc-top-btns"><a href="javascript:void(0);" style="margin-left:10px;" class="addRowBtn"><i class="layui-icon layui-icon-add-circle-fine" style="font-size: 30px; color: &#xe608;"></i> </a> <a href="javascript:void(0);" style="margin-left:10px;" class="queryBtn" ><i class="layui-icon layui-icon-search" style="font-size: 30px; color: &#xe615;"></i> </a><div class="layui-form-item layui-input-inline-block" style="display: inline-block;"><select name="logic" id="logic" lay-verify="required"><option value="and" >并且</option><option value="or">或者</option></select></div></div><div class="conditionDiv layui-form" lay-filter="conditionDiv'+instance.conditionContainerId+'"></div></div>';
...
//    				  layer.msg('校验失败');
    ...
    var dynamicCondition = {
    	...
    	,create:function(config){
    		...
    		selectconditionOption.append("<option value='startTime'>开始时间</option>");
            selectconditionOption.append("<option value='endTime'>结束时间</option>");
